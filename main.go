// Package main implements the Devin Desktop provider plugin for CLIProxyAPI.
//
// The plugin exposes Devin Desktop (formerly Windsurf/Codeium Cascade) as a
// first-class OAuth provider: it performs the browser token handoff login,
// imports credentials from a locally installed Devin Desktop, refreshes session
// tokens, publishes the Devin model catalog, and executes chat requests against
// the Devin Connect RPC endpoint.
package main
...
const (
	// pluginName is the human-readable plugin name reported to the host.
...
func handleMethod(method string, request []byte) ([]byte, error) {
	...
	case pluginabi.MethodCommandLineRegister:
		return handleCommandLineRegister()
...
// callHost invokes a host callback and returns the decoded envelope result.
// It is defined here because the host C shim is file-scoped to this cgo file.
